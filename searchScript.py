...
4. Nothing:\n""")
    ...
    try:
        if option == '1':
            ...
            plotInput = input("\nWhat table(s) are you plotting? \nPress ENTER to include all tables. \nOtherwise, enter the table numbers that you want to include seperated by a comma e.g.) 1,3,5\n")

            if len(plotInput)==1:
                fig = plt.figure()
                ax = fig.add_subplot(111)
                ax.bar(traits, pValLog)
                ax.set_title('Trait vs P-Value: Table '+ plotInput)  
                ax.set_xlabel('Trait')
                ax.set_ylabel('P-Value(-log(P-Value))')

                # graph = plt.bar(traits, pValLog)  
                # plt.title('Trait vs P-Value: Table '+ plotInput)  
                # plt.xlabel('Trait')
                # plt.ylabel('P-Value(-log(P-Value))')
                plt.show()

            elif ',' in plotInput:
                multPlots = list(set([int(i) for i in plotInput.split(",")]))
                fig = plt.figure()
                ax = fig.add_subplot(111)
                #fig, ax = plt.subplots()
                ax.bar(traits, pValLog)
                fig.suptitle('Trait vs -log(P-Value): Tables '+ plotInput)  
                plt.xlabel('Trait')
                plt.ylabel('P-Value')

                #change geo to make room for new subplot
                n = len(multPlots) 
                for i in range(n):
                    fig.axes[i].change_geometry(n+1, n+1, i+1)

                 # add the new
                    ax = fig.add_subplot(n+1, n+1, n+1)
                    ax.bar(traits, pValLog)
                    plt.xlabel('Trait')
                    plt.ylabel('P-Value')
                    #ax.set_ylabel('P-Value(-log(P-Value))'
